...
To test the configuration, create a simple ~index.ts~ file in an
~src/~ folder at the root of the project.  You can type
~console.log('test')~.  After, run grunt in a command line at the root
of your project as well.  This will create a build folder with an
...
#+RESULTS: test-grunt-configuration
